  ...
  handler: async (ctx, args) => {
    ...
    switch (sortBy) {
      ...
      case "name":
        ...
        const nameResults = await query.paginate(paginationOpts);
        const sortedPage = [...nameResults.page].sort((a, b) => a.name.localeCompare(b.name));
        return {
          ...nameResults,
          page: sortedPage,
        };
    ...
    // Handle tag filtering after pagination for better performance
    let results = await query.paginate(paginationOpts);

    if (tags && tags.length > 0) {
      // Filter results to include only products that have at least one matching tag
      results = {
        ...results,
        page: results.page.filter(product =>
          product.tags && product.tags.some(productTag => tags.includes(productTag))
        )
      };
    }
